...
src/main.py       桌面版入口
scripts/          冒烟测试、网页入口和 Pygbag/Vercel 构建脚本
...
1. 克隆仓库，或在 Gitee 页面点击“克隆/下载”获取源码。
2. 创建虚拟环境并安装依赖。
3. 运行游戏入口。
...
运行完整冒烟测试，检查素材、存档迁移、主要界面和全部 15 关：
...
其他用户克隆该目录后会从零创建自己的本地账户。提交代码前可运行 `python scripts/verify_release.py <发布目录>`，再次检查发布目录中是否存在存档或缓存文件。

## 参与贡献

1. Fork 本仓库并从 `main` 创建功能分支，例如 `feat/new-level` 或 `fix/save-migration`。
2. 修改代码后运行 `python scripts/smoke_test.py`。
