...
- **十五关双难度**：每关包含普通与困难模式，普通通关后解锁本关困难及下一关普通。
- **十种肉肉守卫**：远程、阻挡、爆炸、减速、治疗、穿透和资源生产各有定位。
- **八种两阶僵尸**：从偷串宝宝、外卖僵尸到巨人和夜市尸王；二阶会强化生命、攻击与移速。
- **油瓶大招**：每种肉肉都有不同的大招效果，可在局内逆转高压战况。
- **成长与编队**：金币购买新肉肉，每种肉肉拥有专属 7 级升级路线，每关选择最多六个单位出战。
- **本地持久化**：账户、关卡、金币、购买、升级、编队与调参自动保存在本地。
- **便于平衡调试**：每关拥有独立资源、肉肉和僵尸参数，不会污染其他关卡。

## 软件架构

项目采用数据配置、游戏状态、实体行为、资源加载和存档服务分层的方式组织：

```text
src/settings.py   单位、僵尸、双难度、15 关波次和默认平衡参数
src/entities.py   肉肉、僵尸、投射物、拾取物和战斗特效
src/game.py       状态机、界面、关卡流程、商店、升级和特殊玩法
src/assets.py     图片、字体、背景音乐和音效加载
src/save_data.py  桌面 JSON 存档与网页版 localStorage 适配
src/main.py       桌面版入口
scripts/          网页入口和 Pygbag/Vercel 构建脚本
assets/           游戏图片、字体、音乐、音效和授权说明
docs/screenshots/ README 使用的真实游戏截图
```

桌面版通过 Pygame 直接运行；网页版通过 Pygbag 将同一套 Python 游戏代码构建为 WebAssembly。游戏数据只保存在玩家本机：桌面版使用 `save_data.json`，网页版使用当前浏览器的 `localStorage`。
...
1. 克隆仓库，或在 GitHub 页面选择 `Code > Download ZIP` 获取源码。
2. 创建虚拟环境并安装依赖。
3. 运行游戏入口。
...
1. 首次启动输入仅包含英文字母或数字的本地用户名。
2. 点击“开始游戏”，选择已经解锁的关卡及普通/困难模式。
3. 普通模式通关后，会解锁本关困难模式和下一关普通模式。
4. 进入关卡前选择最多 6 种已拥有肉肉；第 7、11 关使用各自的特殊编队规则。
5. 战斗中收集炭火、部署肉肉、使用烧烤夹调整阵容，并在需要时使用油瓶大招。
...
- 第 11 关为僵尸夜宵突袭：系统开局随机摆好固定肉肉防线，玩家用 300 左右的有限军费购买并拖放全部 8 类僵尸；吃肉返还炭火，无时间限制，三次突破烤炉获胜。
...
- 每关提供普通和困难模式。第 1-10 关普通仅有一阶僵尸、困难混合一阶与二阶；第 11-15 关普通混合两阶、困难全部为二阶且增加波次兵力，困难金币奖励为普通的 1.8 倍。
...
- 游戏中可点击顶部 `退出`，经二次确认后返回关卡选择，不会误判失败或发放通关奖励。
- 通关或失守会先播放烧烤摊胜负过场，再显示本关金币、重玩、可用的下一关和返回关卡选择选项。
...
- 每个账户独立保存金币、已解锁关卡、普通/困难通关状态和最近选择的难度。
- 每个账户独立保存已购买肉肉、7 级专属技能升级和最近编队。
...
GitHub 仓库及公开发布包不包含以下本机内容：

- `save_data.json`、临时存档和存档备份；
- `.git`、`.venv`、`.vercel` 等本机环境目录；
- `build`、`dist`、`outputs` 和 Python 缓存；
- 本地账户名、金币、升级、购买记录、调参值及通关进度。

其他用户克隆或下载项目后会从零创建自己的本地账户。

4399、itch.io 等网页游戏平台不能直接运行 Python 源码，需要上传包含 `index.html` 的 HTML5 ZIP 包。生成方式：

```powershell
python -m pip install -r requirements-web.txt
python scripts/build_web.py
Compress-Archive -Path build\webapp\build\web\* -DestinationPath dist\rou-rou-night-market-defense-html5.zip -Force
```

上传时使用 `dist/rou-rou-night-market-defense-html5.zip`，并确保 ZIP 根目录直接包含 `index.html`，不要再套一层文件夹。

## 参与贡献

1. Fork 本仓库并从 `main` 创建功能分支，例如 `feat/new-level` 或 `fix/save-migration`。
2. 修改代码后至少启动桌面版，并检查相关关卡和界面。
3. 不要提交 `save_data.json`、虚拟环境、构建缓存或来源不明的素材。
4. 提交时说明修改内容、测试结果；界面改动请附前后截图。
5. 创建 Pull Request，等待代码与素材授权检查。

欢迎提交关卡创意、数值平衡建议、问题复现步骤和跨平台运行反馈。
